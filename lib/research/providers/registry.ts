...
// Every provider the orchestrator can select from today. `tavily` and
// `brave` are real (Milestone 5); the rest remain typed placeholders
// (Milestone 4). Bing, company-website, and government-dataset providers
// are already valid `ProviderId`s (see schemas/enums.ts) — adding their
// real modules later means writing the file and adding one entry here,
// nothing else.

...
import { businessStage } from "@/lib/pipeline/stages/business";
import type { RankedSource, ResearchResult } from "@/lib/research";

// Milestone 89 — businessStage's wrapped function, discoverBusiness, has
// four cross-package dependencies: runResearch (directly),
// discoverCompetitors, discoverMarket, and discoverFinancials. Mocking
// only the true boundary lets all four real functions run. A thin
...
describe("businessStage", () => {
  it("has the name 'business'", () => {
    expect(businessStage.name).toBe("business");
  });

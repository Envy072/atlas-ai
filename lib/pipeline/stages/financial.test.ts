...
import { financialStage } from "@/lib/pipeline/stages/financial";
import type { RankedSource, ResearchResult } from "@/lib/research";

// Milestone 88 — financialStage's wrapped function, discoverFinancials,
// has three cross-package dependencies: runResearch (directly),
// discoverCompetitors, and discoverMarket. Mocking only the true boundary
// lets all three real functions run. A thin wrapper whose observable
// behavior is limited to constructing the expected request shape and
// delegating to an already-tested platform function.
...
describe("financialStage", () => {
  it("has the name 'financial'", () => {
    expect(financialStage.name).toBe("financial");
  });
